...
createApp({
  data() {
    return {
      ...
      tat: 0,
      eslora: 0,
      url: "https://Ultrasea.pythoneverywhere.com/buques/" + id,
    };
  },
  methods: {
    fetchData(url) {
    ...
    modificar() {
      /* Este código define la función modificar(). En esta función, se crea un objeto buque que contiene las propiedades matricula, nombre, tipo, eslora, tat e imagen obtenidas de la instancia de Vue. Luego se define un objeto options que especifica las opciones para la solicitud HTTP.
      ...
      fetch(this.url, options)
        .then(function () {
          ...
          window.location.href = "../html/buques.html";
        })
        .catch((err) => {
          console.error(err);
